...
/**
 * Advanced State-Machine Terminal Simulator matching iSearch CLI™ standard
 */
function initTerminalMockupTyping() {
    const cursorInput = document.getElementById('cli-interactive-input');
    const bootSection = document.getElementById('cli-boot-section');
    const mainAppSection = document.getElementById('cli-main-app-section');
    const donationSection = document.getElementById('cli-donation-section');
    const helpSection = document.getElementById('cli-help-section');
    const addressVal = document.querySelector('.cli-tui-addr-val');
    const statusMsg = document.getElementById('cli-status-msg-indicator');

    // UI tabs inside the simulated window
    const tabs = document.querySelectorAll('.cli-tab');

    if (!cursorInput) return;

    // Build PIX Payload dynamically inside terminal simulation using official params
    // Recipient Name: 'Erik Rodrigues Balisa', City: 'PERUIBE', Value: R$ 10.00, Key: '11925416678'
    let terminalPixPayload = "";
    if (typeof generatePixPayload === 'function') {
        terminalPixPayload = generatePixPayload("11925416678", "10.00", "ERIK RODRIGUES BALISA", "PERUIBE");
    } else {
        // Safe offline/fallback calculation if scripts/js/pix.js is evaluated after or unavailable
        terminalPixPayload = "00020101021226500014br.gov.bcb.pix011111925416678520400005303986540510.005802BR5921ERIK RODRIGUES BALISA6008PERUIBE62070503***6304677E";
    }

    // Assign payload string to input
    const payloadField = document.getElementById('cli-pix-payload-field');
    if (payloadField) {
        payloadField.value = terminalPixPayload;
    }

    // Set up interactive copy inside terminal mockup
    const termCopyBtn = document.getElementById('cli-copy-pix-btn');
    const termCopyStatus = document.getElementById('cli-copy-status-msg');
    if (termCopyBtn && payloadField) {
        termCopyBtn.addEventListener('click', () => {
            navigator.clipboard.writeText(payloadField.value).then(() => {
                if (termCopyStatus) {
                    termCopyStatus.classList.remove('hidden');
                    setTimeout(() => {
                        termCopyStatus.classList.add('hidden');
                    }, 2500);
                }
            });
        });
    ...
    // Render beautiful QR Code inside simulated terminal
    try {
        const qrContainer = document.getElementById('cli-pix-qr-target');
        if (qrContainer && typeof QRCodeStyling !== 'undefined') {
            ...
            miniQr.append(qrContainer);
        }
    } catch (e) {
        console.error("Failed to generate mini QR Code inside simulator", e);
    }

    // Sequence list representing actual use-cases
    // 1. Boots and displays welcome
    // 2. Types "help" -> loads help viewport
    // 3. Types "donate" -> loads donation viewport
    // 4. Types "browse https://download.erikraft.com" -> switches simulated URL & main screen
    const simulationStates = [
        { type: "command", text: "help", duration: 2500, action: showHelpView },
        { type: "command", text: "back", duration: 1500, action: showMainView },
        { type: "command", text: "donate", duration: 4000, action: showDonationView },
        { type: "command", text: "browse https://download.erikraft.com", duration: 4000, action: showBrowseView },
        { type: "command", text: "clear", duration: 1500, action: showMainView }
    ];
    ...
    // Setup boot sequence
    setTimeout(() => {
        // Hide boot log, fade-in main dashboard layout
        if (bootSection) bootSection.classList.add('hidden');
        if (mainAppSection) mainAppSection.classList.remove('hidden');
        if (statusMsg) statusMsg.textContent = "Loaded Main Dashboard";

        // Start interactive command loop simulation
        setTimeout(runSimulationLoop, 1500);
    }, 3200);

    function runSimulationLoop() {
        if (stateIdx >= simulationStates.length) {
            stateIdx = 0; // Restart loop
        }

        const currentState = simulationStates[stateIdx];
        typeCommandText(currentState.text, () => {
            // Trigger state change action
            currentState.action();

            // Wait for specified duration before deleting and advancing to next state
            setTimeout(() => {
                deleteCommandText(() => {
                    stateIdx++;
                    setTimeout(runSimulationLoop, 800);
                });
            }, currentState.duration);
        });
    }

    function typeCommandText(text, callback) {
        let i = 0;
        function typing() {
            if (i < text.length) {
                cursorInput.textContent = text.substring(0, i + 1);
                i++;
                setTimeout(typing, 80 + Math.random() * 60);
            } else {
    ...
    function deleteCommandText(callback) {
        let len = cursorInput.textContent.length;
        function deleting() {
            if (len > 0) {
                cursorInput.textContent = cursorInput.textContent.substring(0, len - 1);
                len--;
    ...
    // State Viewport Changers
    function showMainView() {
        hideAllViews();
        if (mainAppSection) mainAppSection.classList.remove('hidden');
        if (addressVal) addressVal.textContent = "https://download.erikraft.com";
        if (statusMsg) statusMsg.textContent = "Home Dashboard";
        setActiveTab(0);
    }
    ...
    function showHelpView() {
        ...
        if (statusMsg) statusMsg.textContent = "Help Center Screen";
    }

    function showDonationView() {
        hideAllViews();
        if (donationSection) donationSection.classList.remove('hidden');
        if (statusMsg) statusMsg.textContent = "Support/Donation PIX Standard";
    }
    ...
    function showBrowseView() {
        ...
        if (mainAppSection) mainAppSection.classList.remove('hidden');
        if (addressVal) addressVal.textContent = "https://download.erikraft.com";
        if (statusMsg) statusMsg.textContent = "Browsing erikraft.com";
        setActiveTab(1);
    }
